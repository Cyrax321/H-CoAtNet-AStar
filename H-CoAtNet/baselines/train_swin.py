...
def main():
    ...
    if os.path.exists('best_swin_from_scratch.pth'):
        ...
        # A* Final Test (Held-Out, Once) with full metrics -- device-aware
        model.eval()
        import torch.nn.functional as F
        ...
        # A* metrics: Balanced Acc, Kappa, MCC, ECE, AUROC/AUPRC
        try:
        ...
        plt.title('Confusion Matrix - Swin Transformer (Test Held-Out)')
        plt.tight_layout()
        ...
        plot_curves(history)
        import json as _js; _hp = __import__('pathlib').Path('histories'); _hp.mkdir(exist_ok=True); _hp.joinpath(f'history_swin{SUFFIX}.json').write_text(_js.dumps({'model': 'swin', 'history': history}, indent=2))
    else:
