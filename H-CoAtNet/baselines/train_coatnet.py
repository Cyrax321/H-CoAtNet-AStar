...
def main():
    ...
    if os.path.exists('best_coatnet_baseline.pth'):
        ...
        plot_curves(history)
        import json as _js; _hp = __import__('pathlib').Path('histories'); _hp.mkdir(exist_ok=True); _hp.joinpath(f'history_coatnet{SUFFIX}.json').write_text(_js.dumps({'model': 'coatnet', 'history': history}, indent=2))
    else:
