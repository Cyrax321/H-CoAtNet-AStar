...
def main():
    ...
    if os.path.exists('best_efficientnet_model.pth') or os.path.exists('best_efficientnet_scratch_model.pth'):
        ...
        plot_curves(history)
        import json as _js; _hp = __import__('pathlib').Path('histories'); _hp.mkdir(exist_ok=True); _hp.joinpath(f'history_efficientnet{SUFFIX}.json').write_text(_js.dumps({'model': 'efficientnet', 'history': history}, indent=2))
    else:
